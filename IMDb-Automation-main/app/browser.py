...
# db=Database()
# class BrowserOpen(QRComponent):
#     def __init__(self):
#         self.vault_data = QREnv.VAULTS["Movie_Scrapper"]
        


#     def open_browser(self):
#         # sel.open_available_browser(url)
#         sel.open_available_browser(self.vault_data['url'])
#         sel.maximize_browser_window()
    

#     def search_bar(self, movie):
...
#         sel.input_text(search_path,movie)
#         # sel.press_keys(None,'\ue007' )  #\ue007
#         # sel.click_element_when_visible("//button[@id='suggestion-search-button']")

#         sel.click_element(search_icon)
...
from app.connect_database import DatabaseComponent
import logging
...
class BrowserComponent(QRComponent):
    ...
    def open_browser(self):
        try:
            self.url = QREnv.VAULTS["Movie_Scrapper"]["url"]
            # sel.open_browser(url=self.url, browser="chrome")
            sel.open_available_browser(self.url)
            sel.maximize_browser_window()
        except Exception as e:
    ...
    def search_bar(self,movie):
        ...
        # BuiltIn().log_to_console(movies_data)
        ...
        # sel.press_keys(None,'\ue007')  #\ue007
        sel.click_element_when_visible("//button[@id='suggestion-search-button']")
        sel.click_element_when_visible(movie_path)

        try:
            sel.click_element_when_visible(exact_results)
            data_found = True
            ...
            # TV Movie locator instead
            try:
                sel.click_element_when_visible(tv_movie_path)
                sel.click_element_when_visible(exact_results)
                data_found = True

            except:
                data_found = False
                print("No found movie", movie)
        ...
        if latest_movie_element:
            BuiltIn().log_to_console(latest_movie_element)
            sel.click_element_when_clickable(latest_movie_element)
            # print(f"Clicked on the movie with the latest release date: {latest_release_date}")
            print(f"Release data :{latest_release_date}")
            date = latest_release_date
            print(date)
            ...
            #review
            review_xpath = "//a[normalize-space()='User reviews']"
            review_exists = sel.is_element_visible(review_xpath, missing_ok=True)
            if review_exists:
                sel.scroll_element_into_view(review_xpath)
                sel.click_element_when_visible(review_xpath)
                ...
                try:
                    expander_elements = sel.find_elements(expander_xpath)
                    for expander_element in expander_elements:
                        sel.click_element_when_visible(expander_element)
                except: 
                    pass
                ...
                print("No review.")
                sel.scroll_element_into_view(search_path)
